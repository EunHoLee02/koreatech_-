...
def delete_node(root, key):
    if root is None:
        return None  # 트리가 비어있는 경우

    # 삭제할 키를 탐색
    if key < root.data:  # 삭제할 키가 현재 노드보다 작으면 왼쪽 탐색
        root.left = delete_node(root.left, key)
    elif key > root.data:  # 삭제할 키가 현재 노드보다 크면 오른쪽 탐색
        root.right = delete_node(root.right, key)
    else:  # 삭제할 노드를 발견한 경우
        # 리프 노드 또는 한쪽 자식만 있는 경우
        if root.left is None:
            return root.right
        elif root.right is None:
            return root.left

        # 두 개의 자식이 있는 경우: 오른쪽 서브트리에서 최소값을 찾음
        min_larger_node = find_min(root.right)
        root.data = min_larger_node.data  # 최소값으로 현재 노드 대체
        root.right = delete_node(root.right, min_larger_node.data)  # 최소값 노드 삭제
