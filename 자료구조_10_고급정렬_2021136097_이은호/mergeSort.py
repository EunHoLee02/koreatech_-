...
def merge_sort(A, left, right, sorted):
    ...
    if left < right:
        ...
        # 왼쪽 부분 리스트 정렬
        comparisons_left, movements_left = merge_sort(A, left, mid, sorted)
        # 오른쪽 부분 리스트 정렬
        comparisons_right, movements_right = merge_sort(A, mid + 1, right, sorted)
        # 두 부분 리스트 병합
        comparisons_merge, movements_merge = merge(A, left, mid, right, sorted)
